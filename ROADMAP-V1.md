...
- [x] **Transitions** — done, and *not* in the runtime. Coordinating removal
      ...
      `createPresence` in @volt/primitives simply holds that condition true
...
```
@volt/primitives   behaviour and accessibility, zero styles
@volt/ui           a default styled set built on those primitives
```
...
The single largest thing on this roadmap. Its own package, `@volt/grid`.
...
collection and roving focus before `@volt/primitives` defines them, so those
...
`@volt/server` — `renderToString` and `renderToStream`.
...
- **Data grid (@volt/grid)** — Loading, empty and error states — the three states a grid spends much of its life in, and the ones AG Grid clones routinely omit; Integrated chrome: toolbar with quick search, column-chooser panel, density selector — the capabilities (hide, resize) are listed but not the UI that drives them; Status/footer bar with aggregates over the selection and the filtered set; Conditional row and cell styling, plus cell/header/detail templates as a first-class API; Context menu integration (copy, copy with headers, export, column operations) — context menu is currently listed only as a standalone overlay; Row spanning / cell merging, and sticky group rows
...
**Buys:** The highest-confidence kB win available, and it compounds across every app. Measured: an app using only `Signal.State` ships 1,957 B gzip from @volt/reactivity; importing `StateSignal` directly ships 1,395 B. That is 562 B gzip of pure dead weight in every single application — ~10% of the 5.8 kB budget — because `export namespace` compiles to a runtime object and `introspectSources`, `introspectSinks`, `hasSinks`, `hasSources`, `unwatched`, `Watcher` and `untrack` all stay reachable. The vite-plugin already rewrites `@Component`/`@Prop`, so the machinery exists.
