...
**Buys:** The highest-confidence kB win available, and it compounds across every app. Measured: an app using only `Signal.State` ships 1,957 B gzip from @voltdev/reactivity; importing `StateSignal` directly ships 1,395 B. That is 562 B gzip of pure dead weight in every single application — ~10% of the 5.8 kB budget — because `export namespace` compiles to a runtime object and `introspectSources`, `introspectSinks`, `hasSinks`, `hasSources`, `unwatched`, `Watcher` and `untrack` all stay reachable. The vite-plugin already rewrites `@Component`/`@Prop`, so the machinery exists.
